...
class StudentWorkSDjpaServiceTest {
    ...
    @Test
    void findByTitle() {
        when(studentWorkRepository.findByTitle(anyString())).thenReturn(exam);
    ...
    void findByTeacherLastName() {
        when(studentWorkRepository.findByTeacherUploader_LastName(anyString())).thenReturn(exam);
    ...
    void findByTeacherFirstAndLastName() {
        when(studentWorkRepository.findByTeacherUploader_FirstNameAndTeacherUploader_LastName(anyString(), anyString())).thenReturn(exam);

        StudentWork found = studentWorkSDjpaService.findByTeacherFirstAndLastName("John", "Smith");
        verify(studentWorkRepository, times(1)).findByTeacherUploader_FirstNameAndTeacherUploader_LastName(anyString(), anyString());
        assertEquals(teacherLastName, found.getTeacherUploader().getLastName());
    ...
    @Test
    void findBySubject() {
        when(studentWorkRepository.findBySubject_SubjectName(anyString())).thenReturn(exam);
    ...
    void findByDescription() {
        when(studentWorkRepository.findByAssignmentType_Description(anyString())).thenReturn(exam);
    ...
    void findByContribution() {
        when(studentWorkRepository.findByContributor(anyBoolean())).thenReturn(exam);
